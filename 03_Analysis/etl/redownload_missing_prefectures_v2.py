"""
欠落した5都道府県のデータを再ダウンロード（修正版）

対象: 島根県、山口県、高知県、佐賀県、大分県
気象台コードをZenn記事から取得した正しいコードに修正
"""
...
print("=" * 80)
print("欠落5都道府県の気象データ再ダウンロード（修正版）")
print("=" * 80)
...
if all_daily_data:
    ...
    df_existing = pd.read_csv(existing_file, skiprows=4, encoding='utf-8-sig')

    # 統合
    df_combined = pd.concat([df_existing, df_new], ignore_index=True)

    # 保存
    output_path = OUTPUT_DIR / "jma_daily_temperature_2023_summer_complete.csv"
    df_combined.to_csv(output_path, index=False, encoding='utf-8-sig')

    print(f"\n[OK] データ統合完了: {output_path}")
    print(f"   元データ: {len(df_existing)} 行")
    print(f"   追加データ: {len(df_new)} 行")
    print(f"   合計: {len(df_combined)} 行")

    # 元のファイルを置き換え
    import shutil
    shutil.move(str(output_path), str(existing_file))
    print(f"\n[OK] 元ファイルを更新: {existing_file}")
