"""
e-Stat APIから都道府県別「65歳以上単独世帯数」をダウンロード
...
print("=" * 80)
print("e-Stat APIから65歳以上単独世帯数をダウンロード")
print("=" * 80)
print(f"統計表ID: {STAT_TABLE_ID}")
print(f"フィルタ: 65歳以上 × 世帯人員1人\n")
...
try:
    ...
    if "GET_STATS_DATA" in data and "STATISTICAL_DATA" in data["GET_STATS_DATA"]:
        ...
        for v in values:
            ...
            if area_code in PREF_CODES:
                pref_name = PREF_CODES[area_code]
                value = v.get("$", "0")
                ...
                print(f"  {pref_name}: {elderly_solo_households:,}")

        # DataFrame化
        df_result = pd.DataFrame(results)

        # 一般世帯総数を既存ファイルから取得して、率を計算
        print("\n" + "=" * 80)
        print("一般世帯総数を取得中...")
        print("=" * 80)
